#include <stdlib.h>
#include <stdio.h>
#include <string.h>

int main()
{
  int i, j, mat1[3][3] = {1,2,3,4,5,6,7,8,9};
  char mat5[3][4] = {'a','b','c','d','e','f','g','h','i','j','k','l'};
  
  for(i = 0; i < 3; i++){
      for(j = 0; j < 3; j++){
          printf("Digite o valor %d %d: ", i, j);
          scanf("%d", &mat1[i][j]);
      }
  }
  for (i = 0; i < 3; i++){
      for(j = 0; j < 3; j++){
          printf("%d ", mat1[i][j]);
      } 
      printf("\n");
  }
    return 0;
}
-------------------------------------------------------------------------------------
#include <stdlib.h>
#include <stdio.h>
#include <string.h>

int main()
{
  int i, j, mat1[3][3] = {1,2,3,4,5,6,7,8,9}, mat2[3][3] = {1,2,3,4,5,6,7,8,9}, mat3[3][3] = {1,2,3,4,5,6,7,8,9};
  char mat5[3][4] = {'a','b','c','d','e','f','g','h','i','j','k','l'};
  
  for(i = 0; i < 3; i++){
      for(j = 0; j < 3; j++){
          mat3[i][j] = mat1[i][j] + mat2[i][j];
      }
  } 
  for (i = 0; i < 3; i++){
      for(j = 0; j < 3; j++){
          printf("%2d ", mat3[i][j]);// coloca-se o 2 e imprimi como se tivesse dois algarismos
      } 
      printf("\n");
  }
    return 0;
}
-----------------------------------------------------------------------------------------
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <time.h>

int main()
{
  int i, j, mat1[3][3] = {1,2,3,4,5,6,7,8,9};
  
  srand(time(NULL));
  
  for(i = 0; i < 3; i++){
      for(j = 0; j < 3; j++){
         mat1[i][j] = rand(); 
      }
  }
  for (i = 0; i < 3; i++){
      for(j = 0; j < 3; j++){
          printf("%11d ", mat1[i][j]);// coloca-se o 2 e imprimi como se tivesse dois algarismos
      } 
      printf("\n");
  }
    return 0;
}
--------------------------------------------------------------------------------
#include <stdlib.h>
#include <stdio.h>
#include <string.h>

int main()
{ 
  int i, vetor1[10], vetor2[10];
  
  for(i = 0; i < 10; i++){
      printf("Digite um valor para preencher a posicao %d do vetor: ", i);
      scanf("%d", &vetor1[i]);
  }

  for(i = 0; i < 10; i++){
      vetor2[i] = vetor1[i] * vetor1[i];
  }

    for(i = 0; i < 10; i++){
        printf("%d ", vetor1[i]);
        
    }
    
    printf("\n ");
    
    for(i = 0; i < 10; i++){
        printf("%d ", vetor2[i]); 
    }
    
    return 0;
}
